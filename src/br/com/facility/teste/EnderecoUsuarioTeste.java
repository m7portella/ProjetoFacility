...
//		deletaPorID();
	...
	public static void consultaEndereco(){
		...
		System.out.println(e.getCep().getTipoLogragouro() + " " + e.getCep().getLogradouro() + ", N�" + e.getNumero() + " " + e.getComplemento());
	...
	public static void listarPorUsuario(){
		...
		if(!lstEndereco.isEmpty()){
			for (EnderecoUsuario end : lstEndereco) {
				...
				System.out.println(end.getCep().getTipoLogragouro() + " " + end.getCep().getLogradouro() + ", N�" + end.getNumero() + " " + end.getComplemento());
			}
		}
	}
}
